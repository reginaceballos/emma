...
def make_bayes_model(combined_responses, label_array):
    ...
    y_pred_proba = model_NB.predict_proba(xtest_count)[:, 1] # for ROC curve
    predictions = model_NB.predict(xtest_count)
    flat_y_test = [item for row in y_test for item in row]

    return (model_NB, predictions, flat_y_test, vectorizer, y_pred_proba, y_test)
...
if __name__ == "__main__":
    ...
    # Make the model
    (model_NB, predictions, flat_y_test, vectorizer, y_pred_proba, y_test) = make_bayes_model(combined_responses, label_array)
